...
public class RentACatImpl implements RentACat {
	...
	public boolean rentCat(int id) {
		// TODO: Fill in
		Cat c = getCat(id);
		if (c != null && !c.getRented()) { // the cat with the id exists in the list of cats and has not been rented out
			c.rentCat();
	...
	public String listCats() {
		...
		for (Cat c : cats) {
			if (!c.getRented()) { // only add cats who are available to be rented
				catstring.append(c.toString()).append("\n");
			}
		}
